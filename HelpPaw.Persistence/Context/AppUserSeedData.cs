...
namespace HelpPaw.Persistence.Context
{
    public class UserSeedData
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            // Eğer veritabanında herhangi bir kullanıcı varsa seed işlemini atla
            if (_userManager.Users.Any()) return;


            // ------------------------------------------------------------
            // 2. VETERİNERLERİN OLUŞTURULMASI (Vet Sınıfı Kullanılarak)
            // ------------------------------------------------------------
            var veterinerler = new List<Vet>
            {
                new Vet {
                    // --- IdentityUser Özellikleri ---
                    UserName = "ahmet.vet@klinik.com",
                    Email = "ahmet.vet@klinik.com",
                    EmailConfirmed = true,
                    // Artık VeterinaryPhone yok, klinik numarasını ana telefon yapıyoruz:
                    PhoneNumber = "0216 345 88 90", 
                    
                    // --- AppUsers (Ortak) Özellikleri ---
                    FullName = "Ahmet Yılmaz",
                    City = "İstanbul",
                    CreatedDate = DateTime.UtcNow,
                    IsDeleted = false,
                    // Vet olduğu için bu Location ARTIK KLİNİK KONUMUDUR:
                    Location = new Location(40.9632, 29.0654), 

                    // --- Vet (Özel) Özellikleri ---
                ...
                new Vet {
                    ...
                    EmailConfirmed = true,
                    PhoneNumber = "0312 444 55 66", // Klinik Telefonu
            ...
            {
                // IdentityCreate işlemi AppUser tipini kabul eder, Vet gönderdiğimiz için
                // EF Core bunu "Vets" tablosuna da kaydeder.
            ...
            // ------------------------------------------------------------
            // 3. NORMAL KULLANICILARIN OLUŞTURULMASI (User Sınıfı Kullanılarak)
            // ------------------------------------------------------------
            var kullanicilar = new List<User> // Dikkat: Burada 'User' (PetOwner) sınıfını kullanıyoruz
            {
                new User {
                    ...
                    EmailConfirmed = true,
                    CreatedDate = DateTime.UtcNow,
                    // User olduğu için bu Location EV/ŞAHSİ KONUMDUR:
